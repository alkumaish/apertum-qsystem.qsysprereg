...
public class NetCommander {
     ...
     * @return XML-ответ
     */
    synchronized public static String send(INetProperty netProperty, String commandName, CmdParams params) throws Exception {
        jsonRpc.setMethod(commandName);
        jsonRpc.setParams(params);
        return sendRpc(netProperty, jsonRpc);
    }

    synchronized public static String sendRpc(INetProperty netProperty, AJsonRPC20 jsonRpc) throws Exception {
        ...
        try {
            ...
            throw new Exception("Невозможно получить ответ от сервера. ", ex);
        }
        gson = GsonPool.getInstance().borrowGson();
        try {
            final JsonRPC20Error rpc = gson.fromJson(data, JsonRPC20Error.class);
            if (rpc == null) {
                throw new Exception("Ошибка на сервере не позволила сформировать ответ.");
            }
            if (rpc.getError() != null) {
                throw new Exception("Выполнение задания произошло с ошибкой. " + rpc.getError().getCode() + ":" + rpc.getError().getMessage());
            }
     ...
     * @throws Exception
     */
    public static ServiceState aboutService(INetProperty netProperty, long serviceId) throws Exception {
        //QLog.l().logger().info("Встать в очередь.");
        // загрузим ответ
        final CmdParams params = new CmdParams();
        params.serviceId = serviceId;
     ...
     * @throws Exception
     */
    public static int aboutServicePersonLimitOver(INetProperty netProperty, long serviceId, String inputData) throws Exception {
